...
const BangNghe = () => {
    ...
    const SuaNganhNghe = () => {
        instance({
            url: `https://training.bks.center/api/field/${selected.id}?code=${selected.code}&name=${selected.name}`,
            method: 'PUT',
        }).then(result => {
            CloseModalSua()
            getData()
            ...
            <div>
                <table className='w-full BangNganhNghe'>
                    ...
                    <tr className='border'>
                        ...
                        <th className='border '><input type="text" placeholder="Tìm Mã Ngành Nghề"
                            onChange={(event) => {
                                setSearch(event.target.value)
                            }}></input></th>
                        <th className='border '><input type="text" placeholder="Tìm Tên Ngành Nghề"
                            onChange={(event) => {
                    ...
                    {DanhSachState
                        .filter((danhsach) => {
                            if (searchState === "") {
                                return danhsach
                            } else if (danhsach.code.toLowerCase().includes(searchState.toLocaleLowerCase())) {
                                return danhsach
                            }
                        })
                        .filter((danhsach) => {
                        ...
                        .map((danhsach, index) => (
                            <tr className='border' key={index}>
                                ...
                                <th className='border CotMaNganh '><h1>{danhsach.code}</h1></th>
            ...
            <div className="ModalSua hidden" id="SuaNghe">
                ...
                <div className="flex flex-row">
                    <div>
                        ...
                        <input type="text" placeholder="Mã ngành nghề" className="t313" value={selected.code} name="code" onChange={(e) => handleChange(e)} ></input>
                    </div>
                    <div>
                        <h1 className="t314"> *Ngành nghề </h1>
                        <input type="text" placeholder="Nhập ngành nghề" className="t315" value={selected.name} name="name"  onChange={(e) => handleChange(e)} ></input>
                    </div>
                </div>
