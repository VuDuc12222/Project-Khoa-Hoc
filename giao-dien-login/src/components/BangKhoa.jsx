...
const BangKhoa = () => {
    ...
    const SuaNganhNghe = () => {
        instance({
            url: `https://training.bks.center/api/course/${selected.id}?year=${selected.year}&name=${selected.name}`,
            method: 'PUT',
        }).then(result => {
            CloseModalSua()
            getData()
            ...
            <div>
                <table className='w-full BangNganhNghe'>
                    <tr className='border'>
                        ...
                        <th className='border '>Khóa</th>
                    </tr>
                    <tr className='border'>
                        <th className='border CotID ' colspan="2">Tìm kiếm</th>
                        <th className='border '><input type="text" placeholder="Tìm Khóa"
                            onChange={(event) => {
